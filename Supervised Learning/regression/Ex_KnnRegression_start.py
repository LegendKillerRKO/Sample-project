...
def display_df_info(df_name, my_df, v=False):
    ...
    # Display other optional information with the (v)erbose flag
    if v:
...
class GetAge(BaseEstimator, TransformerMixin):
    ...
    def transform(self,X):
        ...
        """Replace the 'YearBuilt' column values with the calculated age (subtract the 
...
def main():
    ...
    file_path = "train.csv" # Modify to path of file
    input_data = pd.read_csv(file_path, index_col=0)# Read in the input csv file using pandas
    ...
    feature_names = ['LotArea', 'YearBuilt', '1stFlrSF', '2ndFlrSF', 'FullBath', 'BedroomAbvGr', 'TotRmsAbvGrd', 'HouseStyle'] # Define the names of the columns to keep
    ...
    numerical_features = ['LotArea', 'YearBuilt', '1stFlrSF', '2ndFlrSF', 'FullBath', 'BedroomAbvGr', 'TotRmsAbvGrd'] # Define numerical column names
    categorical_features = ['HouseStyle'] # Define categorical column names
    ...
    pipeline = make_pipeline(
        preprocess,
        # Replace with ML algorithm from scikit
        KNeighborsRegressor()
    )
    ...
    """
    x_train, x_test, y_train, y_test = train_test_split(features, output_var, test_size=0.3, random_state=42) # Complete the code
